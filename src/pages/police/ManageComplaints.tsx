...
const FILTER_DISPLAY_KEY: Record<string, string> = {
  ...
  Submitted: "policeManageComplaints.filterSubmitted",
  "Under Review": "myComplaints.statusUnderReview",
  "FIR Drafted": "policeManageComplaints.filterFirDrafted",
...
export default function ManageComplaints() {
  ...
  const filtered = complaints.filter((c) => {
    const matchesFilter = matchesStatusFilter(c, activeFilter);
    const complainantName =
      c.victimName || c.victim?.name || (c.user && typeof c.user === "object" ? c.user.name : "") || "";
    const term = search.toLowerCase();
    const matchesSearch =
      c.complaintId.toLowerCase().includes(term) ||
      complainantName.toLowerCase().includes(term) ||
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex">
      ...
      <main className="flex-1 p-6 lg:p-8 overflow-y-auto">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            <h1 className="text-2xl font-bold text-slate-900">{t("policeManageComplaints.title")}</h1>
            <p className="text-sm text-slate-500 mt-1">
              {t("policeManageComplaints.subtitle").replace("{n}", String(complaints.length))}
            </p>
          </div>
          <button
            onClick={loadComplaints}
            title={t("policeManageComplaints.refreshTitle")}
            className="flex items-center gap-2 text-sm text-slate-600 bg-white border border-slate-300 rounded-lg px-3.5 py-2 shadow-xs hover:bg-slate-50 transition cursor-pointer self-start sm:self-auto"
          >
            <RefreshCw size={15} className={loading ? "animate-spin" : ""} />
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs">
          <div className="flex flex-col lg:flex-row gap-3 mb-5">
            <div className="relative flex-1">
              ...
              <input
                ...
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                placeholder={t("policeManageComplaints.searchPlaceholder")}
                className="w-full border border-slate-300 rounded-lg pl-9 pr-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-700"
              />
            </div>
            <div className="flex items-center gap-2 overflow-x-auto pb-1">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-slate-400 text-left border-b border-slate-200 text-xs">
                  <th className="pb-3 font-semibold">{t("policeDashboard.colId")}</th>
              ...
              <tbody>
                {loading && complaints.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="text-center py-12 text-slate-400">
                      <RefreshCw className="animate-spin mx-auto mb-2 text-blue-800" size={20} />
                      {t("policeDashboard.loadingComplaints")}
                    </td>
                  </tr>
                ) : filtered.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="text-center py-10 text-slate-400 text-sm">
                      {search || activeFilter !== "All"
                        ? t("policeManageComplaints.noMatchFilter")
                        : t("policeManageComplaints.noneInDatabase")}
                    </td>
                  </tr>
                ) : (
                  filtered.map((c) => {
                    ...
                    return (
                      <tr key={c.complaintId} className="border-b border-slate-50 hover:bg-slate-50/60 transition">
                        <td className="py-3.5 font-mono font-semibold text-slate-800">{c.complaintId}</td>
                        <td className="py-3.5 text-slate-600">{complainantName}</td>
                        <td className="py-3.5 text-slate-600 capitalize">{c.incidentType || t("policeDashboard.general")}</td>
                        <td className="py-3.5 text-slate-500 truncate max-w-[140px]">{c.location || "Delhi"}</td>
                        <td className="py-3.5">
                        ...
                        <td className="py-3.5">
                          ...
                          >
                            <Eye size={14} />
                            {t("policeManageComplaints.review")}
                          </button>
                        </td>
                      </tr>
                    );
