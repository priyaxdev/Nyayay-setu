...
export default function Profile() {
  ...
  const displayName = user?.name || t("profile.defaultName");
  const displayEmail = user?.email || t("profile.notProvided");
  const displayPhone = user?.phone || "+91 98765 43210";
  const displayRole = user?.role === "POLICE" ? t("profile.rolePolice") : t("profile.roleCitizen");
  const joinedDate = user?.createdAt
    ? new Date(user.createdAt).toLocaleDateString("en-IN", { month: "long", year: "numeric" })
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-7 mb-5 shadow-xs">
          ...
          <div className="flex items-center gap-4 mb-7">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 mt-0.5">{t("profile.memberSince")} {joinedDate}</p>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1.5">{t("profile.addressLabel")}</label>
              <div className="relative">
                <MapPin className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                <input
                  type="text"
