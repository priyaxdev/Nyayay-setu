...
const FAQ_KEYS = ["faq0", "faq1", "faq2", "faq3", "faq4"];
  ...
  return (
    <div className="min-h-screen bg-slate-100 px-4 py-8">
      <div className="w-full max-w-2xl mx-auto">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-7 mb-5">
          <h1 className="text-2xl font-bold text-slate-900 mb-1">{t("help.title")}</h1>
          <p className="text-sm text-slate-500 mb-6">
            {t("help.subtitle")}
          </p>
            ...
            {FAQ_KEYS.map((key, i) => (
              <div key={key} className="border border-slate-200 rounded-lg overflow-hidden">
                <button
                  ...
                  className="w-full flex items-center justify-between px-4 py-3.5 text-left"
                >
                  <span className="text-sm font-medium text-slate-800">{t(`help.${key}Q`)}</span>
                  <ChevronDown
                    className={`text-slate-400 flex-shrink-0 ml-3 transition-transform ${
                      openIndex === i ? "rotate-180" : ""
                    }`}
                ...
                {openIndex === i && (
                  <div className="px-4 pb-4">
                    <p className="text-sm text-slate-500 leading-relaxed">{t(`help.${key}A`)}</p>
                  </div>
                )}
              </div>
            ))}
        ...
        {/* Contact options */}
        <div className="bg-white border border-slate-200 rounded-2xl p-7">
          <h2 className="font-semibold text-slate-800 mb-4">{t("help.stillNeedHelp")}</h2>
          <div className="grid grid-cols-3 gap-3 mb-5">
            <div className="border border-slate-200 rounded-xl p-4 text-center">
              <Phone className="text-green-800 mx-auto mb-2" size={20} />
              <p className="text-xs font-semibold text-slate-800">{t("help.callUs")}</p>
              <p className="text-xs text-slate-500 mt-1">1800-XXX-XXXX</p>
            </div>
            <div className="border border-slate-200 rounded-xl p-4 text-center">
              <Mail className="text-green-800 mx-auto mb-2" size={20} />
              <p className="text-xs font-semibold text-slate-800">{t("help.email")}</p>
              <p className="text-xs text-slate-500 mt-1">support@suraksha.gov.in</p>
            </div>
            <div className="border border-slate-200 rounded-xl p-4 text-center">
              <MessageCircle className="text-green-800 mx-auto mb-2" size={20} />
              <p className="text-xs font-semibold text-slate-800">{t("help.liveChat")}</p>
              <p className="text-xs text-slate-500 mt-1">9 AM – 9 PM</p>
            </div>
          </div>
