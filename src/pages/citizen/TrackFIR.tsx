...
const STEP_KEYS = [
  { key: "SUBMITTED", labelKey: "trackFir.stepLabel0", descKey: "trackFir.stepDesc0" },
  { key: "UNDER_REVIEW", labelKey: "trackFir.stepLabel1", descKey: "trackFir.stepDesc1" },
  { key: "FIR_DRAFT_GENERATED", labelKey: "trackFir.stepLabel2", descKey: "trackFir.stepDesc2" },
  { key: "OFFICER_VERIFICATION", labelKey: "trackFir.stepLabel3", descKey: "trackFir.stepDesc3" },
  { key: "FIR_REGISTERED", labelKey: "trackFir.stepLabel4", descKey: "trackFir.stepDesc4" },
];
...
export default function TrackFIR() {
  ...
  const loadComplaint = async () => {
    ...
    try {
      ...
      console.warn("Could not load from API:", err);
      setError(t("trackFir.notFoundError"));
      setComplaint({
        complaintId: id,
        user: "demo_user",
        conversationId: "conv_fallback",
  ...
  const currentStep = complaint ? (statusStepIndexMap[complaint.status] ?? 0) : 0;
  const currentStepLabel = t(STEP_KEYS[currentStep]?.labelKey || STEP_KEYS[0].labelKey);
  const badgeStyle = statusBadgeStyles[currentStep] || "bg-slate-100 text-slate-700";
    ...
    <div className="min-h-screen bg-slate-100 px-4 py-8">
      <div className="w-full max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-5">
          ...
          >
            <ArrowLeft size={18} />
            {t("trackFir.backToComplaints")}
          </button>
          <button
            onClick={loadComplaint}
            title={t("trackFir.refreshTitle")}
            className="flex items-center gap-1.5 text-xs text-slate-600 hover:text-slate-900 bg-white border border-slate-300 rounded-lg px-3 py-1.5 shadow-xs transition"
          >
            <RefreshCw size={14} className={loading ? "animate-spin" : ""} />
            {t("trackFir.refresh")}
        ...
        {loading && !complaint ? (
          <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center shadow-xs">
            <RefreshCw className="animate-spin text-green-700 mx-auto mb-3" size={28} />
            <p className="text-sm font-medium text-slate-700">{t("trackFir.fetching")}</p>
          </div>
        ) : (
          <>
            {/* Complaint Header Card */}
            <div className="bg-white border border-slate-200 rounded-2xl p-6 sm:p-7 mb-5 shadow-xs">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6 text-sm bg-slate-50/80 rounded-xl p-4 border border-slate-100">
                <div className="flex items-start gap-2.5">
                  <Shield size={16} className="text-slate-400 mt-0.5 flex-shrink-0" />
                  <div>
                    <p className="text-xs text-slate-400">{t("trackFir.incidentCategory")}</p>
                    <p className="text-slate-800 font-semibold capitalize">
                      {complaint?.incidentType || t("dashboard.genericIncident")}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-2.5">
                  <Calendar size={16} className="text-slate-400 mt-0.5 flex-shrink-0" />
                  <div>
                    <p className="text-xs text-slate-400">{t("trackFir.dateOfIncident")}</p>
                    <p className="text-slate-800 font-semibold">
                      {complaint?.date || t("trackFir.notSpecified")}
                      {complaint?.time ? ` (${complaint.time})` : ""}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-slate-800 font-semibold">
                      {complaint?.location || t("trackFir.notSpecified")}
                    </p>
                  </div>
                </div>
                ...
                {complaint?.stolenItem ? (
                  <div className="flex items-start gap-2.5">
                    <Package size={16} className="text-slate-400 mt-0.5 flex-shrink-0" />
                    <div>
                      <p className="text-xs text-slate-400">{t("trackFir.stolenProperty")}</p>
                      <p className="text-slate-800 font-semibold capitalize">
                        {complaint.stolenItem}
                      </p>
                    </div>
                  ...
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-slate-800 font-semibold">
                        {complaint?.victim?.name || t("trackFir.citizenFallback")}
                      </p>
                    </div>
                  </div>
                )}
              ...
              <div className="border-t border-slate-100 pt-4">
                <div className="flex items-start gap-2.5">
                  <FileText size={16} className="text-slate-400 mt-0.5 flex-shrink-0" />
                  <div>
                    <p className="text-xs text-slate-400 font-medium mb-1">{t("trackFir.incidentStatement")}</p>
                    <p className="text-sm text-slate-700 leading-relaxed italic bg-white p-3 rounded-lg border border-slate-100">
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-6 sm:p-7 shadow-xs">
              <h2 className="font-bold text-slate-900 mb-6 text-base flex items-center justify-between">
                <span>{t("trackFir.timelineTitle")}</span>
                <span className="text-xs font-normal text-slate-400">{t("trackFir.stageOf").replace("{n}", String(currentStep + 1))}</span>
              </h2>
